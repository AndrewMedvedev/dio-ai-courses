...
class SqlCourseRepository:
    ...
    def create(self, payload) -> Course:
    ...
    def create_draft(self, *, title: str, description: str, difficulty: str, tags: list) -> Course:
    ...
    def list(
        ...
        *,
        ...
        offset = (page - 1) * limit
        courses = list(self.session.scalars(query.offset(offset).limit(limit)).all())
    ...
    def create_module_nested(self, course_id: UUID, payload) -> Module:
        """Создание модуля с вложенными уроками и практикой (используется внутри create)."""
    ...
    def add_module(self, course_id: UUID, payload) -> Module:
    ...
    def add_lesson(self, module_id: UUID, payload) -> Lesson:
        """Создать урок модуля в следующей доступной позиции."""

        max_position = self.max_lesson_position(module_id)
        lesson = Lesson(
            module_id=module_id,
            title=payload.title,
    ...
    def add_practice(self, module_id: UUID, payload) -> Practice:
...
class SqlGenerationRepository:
    ...
    def add_task(self, payload) -> CourseGenerationTask:
