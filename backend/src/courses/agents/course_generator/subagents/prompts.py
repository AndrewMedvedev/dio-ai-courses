...
class LessonStructure(BaseModel):
    ...
    assignment_specification: AssignmentSpecification = Field(description=ASSIGNMENT_PROMPT)
...
Чтобы сценарий был полноценным и эффективным, он обязательно должен включать:
 ...
 - **Визуализация:** Описания слайдов, графиков — особенно для онлайн-курсов.
...
Вид ответа — Верни ответ **только** в формате JSON, без дополнительных пояснений, текста или Markdown. Ответ должен быть валидным JSON.
...
CONTENT_BLOCK_PROMPTS = {
    ...
    """,  # ruff:ignore[line-too-long]
    # ContentType.IMAGE: """\
    # Ты — эксперт по составлению текстовых промптов для генерации изображений. Твоя задача — на основе запроса и контекста учебного модуля составить максимально подробный, однозначный и качественный промпт для генерации иллюстрации к образовательному материалу.
    # ### Правила составления промпта
    # 1. Тематика и стиль:
    #     - Изображение должно быть образовательным, наглядным, без визуального шума.
    #     - Если явно не указано иное, используй стиль: чистая векторная иллюстрация,
    #         плоский дизайн (flat design) или схематичная инфографика — это лучше подходит
    #         для учебных материалов, чем фотореализм.
    #     - Избегай стилей, которые могут отвлекать от сути (сюрреализм, абстракция), если контекст явно не требует творческой иллюстрации.
    # 2. Структура промпта (соблюдай порядок):
    #     - Главный объект/сцена: что именно должно быть изображено (конкретно,
    #         без общих фраз вроде "что-то про программирование").
    #     - Ключевые детали: элементы, которые обязательно должны присутствовать
    #         (подписи, элементы интерфейса, конкретные объекты, персонажи, схемы).
    #     - Композиция: расположение объектов (по центру, слева-справа, план сверху и т.д.).
    #     - Цветовая палитра: 2-4 основных цвета, чтобы избежать хаотичной раскраски.
    #     - Стиль отрисовки: (плоский дизайн / инфографика / векторная иллюстрация / 3D-рендер / реалистичное фото — выбери один).
    #     - Настроение/тон: (нейтральный, дружелюбный, минималистичный, технологичный и т.д.).
    # 3. Технические уточнения:
    #     - Пиши промпт на русском языке
    #     - Указывай соотношение сторон в текстовом описании, если это важно
    #         (например, "широкий баннерный формат", "квадратный формат",
    #         "вертикальный формат постера").
    #     - Явно указывай, чего не должно быть на изображении (например: "без текста", "без водяных знаков", "без реалистичных лиц людей" — если это не нужно для сути).
    #     - Если в изображении нужен текст/подписи — указывай ТОЧНЫЙ текст в кавычках и его расположение, так как модели часто искажают текст на изображениях; если точность текста критична, лучше указать "без текста на изображении" и предложить добавить подписи отдельно.
    # 4. Избегай неоднозначности:
    #     - Не используй абстрактные прилагательные без конкретики (не "красивое изображение", а "чистая векторная иллюстрация с мягкими тенями").
    #     - Не перегружай промпт более чем 5-7 ключевыми элементами — это снижает точность генерации.
    # ### Формат ответа
    # Всегда возвращай финальный промпт на русском языке, оформленный единым
    # связным текстом (не списком), без вводных фраз типа "Вот промпт:" —
    # только сам промпт, готовый к передаче в генеративную модель.
    # Пример структуры финального промпта:
    # "Векторная иллюстрация в плоском стиле, изображающая [главный объект],
    # показывающая [ключевые детали], композиция: [расположение],
    # цветовая палитра: [цвета], стиль: [стиль], настроение: [тон],
    # без текста, без водяных знаков, квадратный формат."
    #     """,
    ContentType.QUIZ: """\
