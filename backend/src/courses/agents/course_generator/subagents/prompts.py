...
class LessonStructure(BaseModel):
    ...
    assignment_specification: tuple[AssignmentType, str] = Field(description=ASSIGNMENT_PROMPT)
...
CONTENT_BLOCK_PROMPTS = {
    ...
    """,  # noqa: E501
    ContentType.IMAGE: """\
    Ты — эксперт по составлению текстовых промптов для генерации изображений с помощью AI-модели генерации изображений. Твоя задача — на основе запроса пользователя и контекста учебного модуля составить максимально подробный, однозначный и качественный промпт для генерации иллюстрации к образовательному материалу.

    ### Доступные инструменты
        - knowledge_search - поиск информации в базе знаний курса, используй, если тебе
        не хватает контекста для точного и релевантного промпта (не более 2 вызовов).
